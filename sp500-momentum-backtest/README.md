...
`dashboard/index.html` is a self-contained comparison dashboard (value
chart, drawdown, sleeve composition, allocation, metric bars, tables,
the auto-generated interpretation) for all four strategies. It's built
from `dashboard/demo_data.json`, generated by
`scripts/generate_demo_data.py`, which runs the real `src/` pipeline
(ranking -> portfolio -> diversified -> metrics -> interpretation)
against a fabricated synthetic market -- every number on the dashboard
is genuinely computed by the tested backtest code, only the input
prices are fake, and the page says so in a banner. To regenerate it
with real results once you're on a machine with internet access:

```bash
python main_diversified.py                 # or main.py, if you only need A/B/C
python scripts/generate_demo_data.py        # rebuild dashboard/demo_data.json from real results
```

(`generate_demo_data.py` currently always fabricates its own universe
rather than reading `output/*.csv` -- swap in a loader over those CSVs
if you want the dashboard driven directly by a `main.py`/
`main_diversified.py` run instead of its own synthetic generator.)
...
scripts/generate_demo_data.py  builds dashboard/demo_data.json by running src/ against a fabricated universe
dashboard/index.html      self-contained comparison dashboard (reads dashboard/demo_data.json, embedded inline)
dashboard/demo_data.json  data behind the dashboard -- see scripts/generate_demo_data.py to regenerate
```
